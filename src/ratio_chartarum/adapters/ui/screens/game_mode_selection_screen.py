...
class GameModeSelectionScreen(Screen):
    ...
    NAVIGATION_MAP = {
        BTN_CLASSIC_ID: ClassicModeScreen,
        BTN_RANDOM_ID: RandomModeScreen,
        BTN_INFINITE_ID: InfiniteModeScreen,
        BTN_CUSTOM_ID: CustomModeScreen
    }

    @override
    def compose(self: Self) -> ComposeResult:
        yield Static("Selecciona un modo de juego:", id="mode-title")
        yield Button("Duelo Clásico", id=self.BTN_CLASSIC_ID)
        yield Button("Duelo Aleatorio", id=self.BTN_RANDOM_ID)
        yield Button("Duelo Infinito", id=self.BTN_INFINITE_ID)
        yield Button("Duelo Personalizado", id=self.BTN_CUSTOM_ID)
